...
export async function getPaymentMethod(paymentMethodId: string, accountId: string) {
    let url = `https://api.testwyre.com/v2/paymentMethod/${paymentMethodId}?masqueradeAs=${accountId}&timestamp=${_timestamp}`;
    const response = await axios.get(url, configureGetOptions(url))
        .then(response => response.data)
        .catch(error => console.log(error));
    return response;
...
export async function getAllPaymentMethods(accountId: string) {
    let url = `https://api.testwyre.com/v2/paymentMethods?masqueradeAs=${accountId}&timestamp=${_timestamp}`;
    const response = await axios.get(url, configureGetOptions(url))
        .then(response => response.data)
        .catch(error => console.log(error));
    return response;
...
export async function paymentMethodFollowUp(accountId: string, paymentMethodId: string){
    let url = `https://api.sendwyre.com/v2/paymentMethod/${paymentMethodId}/followup?masqueradeAs=${accountId}&timestamp=${_timestamp}`;
...
export async function attachBlockChainToPaymentMethod(blockChain: 'BTC' | 'ETH' /*or both separated with commas 'ETH', 'BTC'*/, paymentMethodId: string, accountId: string) {
    let url = `${_rootUrl}/v2/paymentMethod/${paymentMethodId}/attach?masqueradeAs=${accountId}&timestamp=${_timestamp}`
    let body = {blockChain: blockChain};
    const response = await axios.get(url, configurePostOptions(url, body))
        .then(response => response.data)
        .catch(error => console.log(error));
    return response;
